...
class MatchList extends Component<PageOwnProps, PageState> {
  ...
  render() {
    const {matchList, loading = false, visible = false, loadingmore = false, nextPage, showRound = true} = this.props
    if (!visible) {
      return <View/>
    }
    if (loading) {
      return <View className="qz-league-manager__match-list-loading">
    ...
    return <View className='qz-league-manager__match-list'>
      {matchTimeList.keys().map((key) => (
        <View key={key} className='qz-league-manager__match-list-content'>
          <View className='qz-league-manager__match-list-content__inner'>
            <Text className="qz-league-manager__match-list-time">{key}</Text>
